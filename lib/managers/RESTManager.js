...
const { DiscordHTTPError, DiscordAPIError } = require("../errors/DiscordErrors");
...
class RESTManager {
    ...
    async __request(route, payload, contentType) {
        ...
        if (payload && payload.reason) {
            let unencodedReason = payload.reason;
            ...
            if ((method !== "PUT" || !route.url.includes("/bans")) && (method !== "POST" || !route.url.includes("/prune"))) {
                delete body.reason;
            } else {
    ...
    _incrRetries(key) {
        const buffer = this.retries.get(key);
