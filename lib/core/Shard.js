...
/**
 * Represents a shard
 * @property {Boolean} connected Whether the shard was connected or not
 ...
 * @property {Number} shardCount The total shard count
...
class Shard {
    constructor(client, shardID) {
        ...
        this.sequence = null;
        this.sessionID = null;
        this.shardCount = this.client.options.shardCount;
        this.ws = undefined;
    ...
    createGuild(_guild) {
        this.client._guildShard[_guild.id] = this.id;
        const guild = new Guild(this.client, _guild);
     ...
     */
    editPresence(status, activity) {
        this.send(JSON.stringify({
            op: Constants.GatewayOPCodes.PRESENCE_UPDATE,
            d: {
                game: {
    ...
    setupWebsocket() {
        ...
        this.ws.on("error", this._wsError);
        this.ws.on("open", this._wsOpen);
    }
        ...
        switch (packet.t) {
            case "READY": {
                ...
                if (!this.id) {
                    ...
                    await this.client.emit("gatewayReady", this.client.user);
                }
    ...
    _wsClose(code, reason) {
        ...
        if (code) {
            if (code === 1006) {
                err = new DiscordError("Connection was closed abnormally");
            } else if (code === 4003) {
                err = new DiscordError("Not authenticated");
                this.sessionID = null;
            }
