...
class Shard {
    ...
    /**
     * Send a payload data to the Websocket
     * @param  {...any} args The payload data
     */
    async send(...args) {
    ...
    /**
     * Setup and initialize the Websocket connection
     */
    setupWebsocket() {
    ...
    /**
     * Process the websocket raw data
        ...
        switch (packet.t) {
            case "READY": {
                ...
                if (!this.id) {
                    ...
                    await this.client.emit("ready");
                }
                break;
            }
            case "RESUMED": {
                await this.client.emit("resumed");
                break;
            }
            case "MESSAGE_CREATE": {
